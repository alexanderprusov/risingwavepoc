...
# To use: run ./gradlew :rw-loader:generateProto once so Gradle downloads the binary, then run this script.
...
if [ -z "$SRC" ]; then
  echo "protoc binary not found in Gradle cache. Run: ./gradlew :rw-loader:generateProto" >&2
  exit 1
fi
