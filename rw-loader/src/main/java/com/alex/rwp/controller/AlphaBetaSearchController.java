...
@RestController
@RequestMapping("/api/alpha-beta")
@Tag(name = "Alpha-Beta Search", description = "Search joined alpha-beta entities via refs")
public class AlphaBetaSearchController {

    private final JdbcTemplate jdbc;

    public AlphaBetaSearchController(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    @GetMapping("/search")
    @Operation(
        summary = "Search joined alpha-beta",
        description = "Returns joined rows matching alpha_name and beta_title (substring). Omit a parameter to skip that filter."
    )
    ...
    @Operation(
        summary = "Search joined alpha-beta (timed)",
